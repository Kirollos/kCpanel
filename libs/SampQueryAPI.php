...
class SampQueryAPI
{
	...
	public function getInfo()
	{
		...
		// Edit by Kirollos: Since 0.3.7 there is a language in "Server Info" query, 'mapname' is moved to "Server Rules" query.
		// More information on http://wiki.sa-mp.com/wiki/Query_Mechanism#Response_from_i

		$iStrlen = ord(fread($this->rSocket, 4));
		if($iStrlen > 0) { // Checks if the server hasn't set language empty
			$aDetails['language'] = (string) fread($this->rSocket, $iStrlen); // kiro edition: mapname => language
		} else {
			$aDetails['language'] = '-';
		}
